export const imgCloudinary = {
  crown:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1737818762/corona_go668r.png',
  mirror:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1737818799/marco_nyfae7.png',
  candle:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1737818777/vela_hdvwla.png',
  stamp:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1737818773/marca_aedbi1.png',
  left: 'https://res.cloudinary.com/djqwo9izl/image/upload/v1737818808/esquina_izq_mtqnhv.png',
  right:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1737818808/esquina_derch_ecdzs6.png',
  angel:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1738622924/angel_lm0mlo.png',
  cupidBoi:
    'https://res.cloudinary.com/djqwo9izl/image/upload/v1739755053/cupido-removebg-preview_q7izdq.png',
};

export const imgInProject = {
  trash: require('../assets/img/trash-icon.png'),
  devil: require('../assets/img/eldiablo.png'),
  logOut: require('../assets/img/log-out.png')
}
